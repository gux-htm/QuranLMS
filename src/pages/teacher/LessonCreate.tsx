...
export function TeacherLessonCreate() {
  ...
  const [arabic, setArabic] = useState('بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ')
  ...
  const preview = useMemo(
    () => ({ title: previewTitle, arabic: arabic.trim() || 'Arabic lesson content' }),
    [previewTitle, arabic],
  )

  const toggleRule = (rule: string) =>
    setRules((curr) => (curr.includes(rule) ? curr.filter((r) => r !== rule) : [...curr, rule]))

  const toggleClass = (classId: string) =>
    setSelectedClasses((curr) =>
      curr.includes(classId) ? curr.filter((id) => id !== classId) : [...curr, classId],
    )
  ...
  const submit = async (mode: 'draft' | 'publish') => {
    ...
    push(mode === 'publish' || publishImmediately ? 'Assignment published' : 'Assignment draft saved')
    setSaving(false)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold text-ink">Create assignment</h1>
        <p className="mt-1 text-sm text-ink/55">Prepare an assignment and preview what students will see.</p>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(300px,.75fr)]">
          ...
          <CardContent className="space-y-5">
            ...
            <label className="block">
              ...
              <input
                ...
                placeholder="e.g. Juz 1, Pages 1–3"
                className="h-10 w-full rounded-xl border border-line bg-white px-3 text-sm text-ink outline-none focus:ring-2 focus:ring-green-600/30"
              />
            </label>
                  ...
                  <button
                    ...
                    className={`rounded-md border px-3 py-2 text-sm ${
                      lessonType === value
                        ...
                        : 'border-line text-ink/55 hover:bg-paper-dim'
                    }`}
                  >
                    {label}
                  </button>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-ink">Transliteration</span>
                <textarea
              ...
              <label className="block">
                <span className="mb-1.5 block text-sm font-medium text-ink">English translation</span>
                <textarea
                  value={translation}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-ink" htmlFor="audio-source">
                  Qari / audio source
                </label>
                <select
                  id="audio-source"
                  value={qari}
                  onChange={(e) => setQari(e.target.value as typeof AUDIO_SOURCES[number])}
                  className="h-10 w-full rounded-xl border border-line bg-white px-3 text-sm text-ink"
                >
                  {AUDIO_SOURCES.map((s) => <option key={s}>{s}</option>)}
                </select>
              </div>
              {qari === 'Custom URL' && (
                <Input
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-ink" htmlFor="target-unit">
                  Target unit
                </label>
                <select
                  id="target-unit"
            ...
            {/* Tajweed rules */}
            <div>
              <span className="mb-2 block text-sm font-medium text-ink">Tajweed rules</span>
              <div className="flex flex-wrap gap-2">
                {RULES.map((rule) => (
                  ...
                  >
                    {rules.includes(rule) && <Check className="mr-1 h-3 w-3" />}
                    {rule}
                  </button>
                ))}
              </div>
            ...
            {/* Assign to class */}
            <div>
              <span className="mb-2 block text-sm font-medium text-ink">Assign to class</span>
        ...
        <Card className="h-fit lg:sticky lg:top-24">
          <CardTitle className="mb-3">Student preview</CardTitle>
          <CardContent>
            <div className="rounded-xl border border-line border-r-4 border-r-green-700 bg-white p-5">
              ...
              {translation && <p className="mt-2 text-xs leading-6 text-ink/45">{translation}</p>}
              <div className="mt-4 flex items-center justify-between border-t border-line pt-3 text-xs text-ink/50">
                <span>Target</span>
                <span className="font-semibold text-ink">{quantity} {unit}</span>
              </div>
              {rules.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {rules.map((rule) => (
                    <span key={rule} className="rounded-full bg-green-50 px-2.5 py-1 text-xs text-green-700">
                      {rule}
                    </span>
                  ))}
                </div>
