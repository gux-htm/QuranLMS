...
export function TeacherLessonCreate() {
  ...
  const [arabic, setArabic] = useState('Ø¨ÙØ³Ù’Ù…Ù Ø§Ù„Ù„ÙŽÙ‘Ù‡Ù Ø§Ù„Ø±ÙŽÙ‘Ø­Ù’Ù…ÙŽÙ€Ù°Ù†Ù Ø§Ù„Ø±ÙŽÙ‘Ø­ÙÙŠÙ…Ù')
        ...
        <Card>
          <CardContent className="space-y-5">
            <label className="block">
              ...
              <input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Juz 1, Pages 1â€“3" className="h-10 w-full rounded-xl border border-line bg-white px-3 text-sm text-ink outline-none focus:ring-2 focus:ring-green-600/30" />
            </label>
            ...
            <label className="block">
              ...
              <textarea dir="rtl" value={arabic} onChange={(event) => setArabic(event.target.value)} rows={7} placeholder="Ø¨ÙØ³Ù’Ù…Ù Ø§Ù„Ù„ÙŽÙ‘Ù‡Ù Ø§Ù„Ø±ÙŽÙ‘Ø­Ù’Ù…ÙŽÙ€Ù°Ù†Ù Ø§Ù„Ø±ÙŽÙ‘Ø­ÙÙŠÙ…Ù" className="w-full rounded-xl border border-line bg-white px-3 py-3 text-right font-[Noto_Naskh_Arabic] text-2xl leading-relaxed text-ink outline-none focus:ring-2 focus:ring-green-600/30" />
            </label>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="block"><span className="mb-1.5 block text-sm font-medium text-ink">English translation</span><textarea value={translation} onChange={(event) => setTranslation(event.target.value)} rows={4} placeholder="Optional English meaning" className="w-full rounded-xl border border-line bg-white px-3 py-2 text-sm text-ink outline-none focus:ring-2 focus:ring-green-600/30" /></label>
            </div>

            <div className="grid gap-4 md:grid-cols-2">
              <div><label className="mb-1.5 block text-sm font-medium text-ink" htmlFor="audio-source">Qari / audio source</label><select id="audio-source" value={qari} onChange={(event) => setQari(event.target.value as (typeof AUDIO_SOURCES)[number])} className="h-10 w-full rounded-xl border border-line bg-white px-3 text-sm text-ink"><option>Sudais</option><option>Al-Minshawi</option><option>Pothwari</option><option>Custom URL</option></select></div>
              {qari === 'Custom URL' && <Input label="Audio URL" value={audioUrl} onChange={(event) => setAudioUrl(event.target.value)} placeholder="https://â€¦" />}
            </div>

            <div className="grid gap-4 sm:grid-cols-2"><div><label className="mb-1.5 block text-sm font-medium text-ink" htmlFor="target-unit">Target unit</label><select id="target-unit" value={unit} onChange={(event) => setUnit(event.target.value as (typeof UNITS)[number])} className="h-10 w-full rounded-xl border border-line bg-white px-3 text-sm text-ink">{UNITS.map((item) => <option key={item}>{item}</option>)}</select></div><Input label="Target quantity" type="number" min="1" step="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></div>
            ...
            <div><span className="mb-2 block text-sm font-medium text-ink">Assign to class</span><div className="grid gap-2 sm:grid-cols-2">{classes.map((item) => <label key={item.id} className="flex items-center gap-2 rounded-xl border border-line px-3 py-2 text-sm text-ink"><input type="checkbox" checked={selectedClasses.includes(item.id)} onChange={() => toggleClass(item.id)} />{item.name}</label>)}</div></div>
