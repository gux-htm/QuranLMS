...
export function StudentAssignments() {
  ...
  return <div>
    ...
    {!empty && <div className="mt-6 inline-flex rounded-md bg-paper-dim p-1">{(['Pending', 'Completed', 'All'] as const).map((t) => <button key={t} onClick={() => setTab(t)} className={`rounded px-4 py-2 text-sm ${tab === t ? 'bg-white text-green-700 shadow-sm' : 'text-ink/55'}`}>{t}</button>)}</div>}
    <div className="mt-5">{empty ? <EmptyState icon={<BookOpen />} title="All caught up" description="Your teacher hasn't assigned any lessons yet." /> : list.length === 0 ? <EmptyState icon={<BookOpen />} title="Nothing here yet" description="Your assignments will appear here when your teacher sends them." /> : <div className="space-y-3">{list.map((a) => <Card key={a.id}><CardContent className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>{a.title}</CardTitle><p className="mt-1 text-xs text-ink/55">Assigned {a.assigned} · Due {a.deadline}</p></div><div className="flex items-center gap-3"><span className={`rounded-full px-2.5 py-1 text-xs ${a.status === 'completed' ? 'bg-green-50 text-green-700' : a.status === 'in_progress' ? 'bg-sky-100 text-sky-700' : 'bg-gold-100 text-gold-800'}`}>{a.status === 'in_progress' ? 'In progress' : a.status === 'completed' ? 'Completed' : 'Pending'}{a.score ? ` · ${a.score}/100` : ''}</span><Button size="sm" variant={a.status === 'completed' ? 'outline' : 'default'} onClick={() => navigate(`/student/lesson/${a.id}`)}>{actionLabel(a.status)}</Button></div></CardContent></Card>)}</div>}</div>
  </div>
}
