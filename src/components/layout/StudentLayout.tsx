    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      >
        ...
        <nav className="space-y-1" aria-label="Student navigation">
          <p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-ink/35">
            Workspace
          </p>
          {navItems.map((item) => {
              ...
              >
                <Icon
                  className={`h-[18px] w-[18px] ${
                    active ? 'text-paper' : 'text-ink/45 group-hover:text-green-700'
                  }`}
                />
                {item.label}
              </button>
        ...
        <div className="mt-auto space-y-1 border-t border-line pt-4">
          ...
          >
            <Settings
              className={`h-[18px] w-[18px] ${
                isActive('/student/settings') ? 'text-paper' : 'text-ink/45'
              }`}
            />
            Settings
          </button>
          <button
            onClick={() => navigate('/')}
            className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-ink/55 transition-colors hover:bg-clay-100 hover:text-clay-800"
          >
          ...
          <div className="flex h-[72px] items-center justify-between gap-4 px-4 sm:px-7">
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:block">
                <p className="text-xs text-ink/40">{CURRENT_STUDENT.className}</p>
                <p className="font-display text-lg font-semibold">Student workspace</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-line bg-white py-1.5 pl-1.5 pr-3 shadow-sm">
                ...
                <div className="hidden sm:block">
                  ...
                  <div className="text-[11px] text-ink/45">Rank #{CURRENT_STUDENT.rank}</div>
                </div>
              </div>
            </div>
          </div>
